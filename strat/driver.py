...
subprocess.run(["python3" ,"strat/ED24B017.py"], capture_output=True)
...
for t in range(10, 750, 10):
    ...
    with open("results/summary.txt", "r") as f:
        lines = f.readlines()
        df.append([t] + [line.split(":")[1].strip() for line in lines])

    columns = ["t", "final_value", "total_trades", "max_drawdown", "volatility", "sharpe"]
